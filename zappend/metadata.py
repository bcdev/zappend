...
class DatasetMetadata:
    ...
    def from_dataset(cls,
        ...
        attrs = merge_configs(dataset.attrs,
                              config.get("attrs") or {})
...
def _get_effective_dims(dataset: xr.Dataset,
                        config_fixed_dims: dict[str, int] | None,
                        config_append_dim: str) -> dict[str, int]:
...
def _get_effective_variables(dataset: xr.Dataset,
    ...
    if unknown_var_names:
        raise ValueError(f"The following variables are neither configured"
    ...
    for var_name in selected_var_names:
        ...
        if ds_var is not None:
            ...
            ds_var_def = dict(dims=tuple(map(str, ds_var.dims)),
                              shape=ds_var.shape,
            ...
            for dim in config_var_dims:
                if dim not in dataset.dims:
                    raise ValueError(f"Dimension {dim!r} of variable"
                                     f" {var_name!r} not found in dataset")
            config_var_def["shape"] = tuple(dataset.dims[k]
                                            for k in config_var_dims)
            encoding: dict | None = config_var_def.get("encoding")
            if encoding is None or encoding.get("dtype") is None:
                raise ValueError(f"Missing 'dtype' in encoding configuration"
                                 f" of variable {var_name!r}")
