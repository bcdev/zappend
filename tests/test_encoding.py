...
class XArrayEncodingTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.ds = xr.Dataset(
            data_vars=dict(
                v=xr.DataArray(np.ones(100, dtype=np.float64),
                               dims="x")
            ),
            coords=dict(
                x=xr.DataArray(np.linspace(0, 1, 100, endpoint=False,
                                           dtype=np.float32),
                               dims="x")
            ),
        )
        ...
        self.ds_2 = xr.Dataset(
            data_vars=dict(
                v=xr.DataArray(np.full(100, 2.0, dtype=np.float64),
                               dims="x")
            ),
            coords=dict(
                x=xr.DataArray(np.linspace(1, 2, 100, endpoint=False,
                                           dtype=np.float32),
                               dims="x")
            ),
        )
    ...
    def test_no_encoding_given(self):
        ...
        self.assertEqual(self.ds_v_attrs, get_v_attrs())
        self.assertEqual({**self.ds_v_meta,
                          "shape": [200],
                          "chunks": [100]},
                         get_v_meta())
    ...
    def test_chunks_in_encoding_kwargs_works(self):
        self.ds.to_zarr(TEST_ZARR,
                        encoding=dict(v=dict(chunks=(20,))))
        self.assertEqual(self.ds_v_attrs, get_v_attrs())
        self.assertEqual({**self.ds_v_meta,
                          "chunks": [20]},
                         get_v_meta())
    ...
    def test_chunks_in_kwargs_override_v_encoding(self):
        self.ds.v.encoding.update(chunks=(20,))
        self.ds.to_zarr(TEST_ZARR,
                        encoding=dict(v=dict(chunks=(30,))))
        self.assertEqual(self.ds_v_attrs, get_v_attrs())
        self.assertEqual({**self.ds_v_meta,
                          "chunks": [30]},
                         get_v_meta())
    ...
    def test_packing_in_v_attrs_has_wrong_effect(self):
        self.ds.v.attrs.update(scale_factor=0.001,
                               add_offset=5)
        self.ds.v.encoding.update(dtype=np.int16)
        self.ds.to_zarr(TEST_ZARR)
        self.assertEqual({**self.ds_v_attrs,
                          "scale_factor": 0.001,
                          "add_offset": 5},
                         get_v_attrs())
        self.assertEqual({**self.ds_v_meta,
    ...
    def test_packing_in_v_encoding_works(self):
        self.ds.v.encoding.update(scale_factor=0.001,
                                  add_offset=5,
                                  dtype=np.int16)
        self.ds.to_zarr(TEST_ZARR)
        self.assertEqual({**self.ds_v_attrs,
